...
#creating a Flask Group function which is used to create co mands to manage the flask application
cli = FlaskGroup(create_app=create_app)

#data of the users used to seed the database
user_json = [
	{
		"name": "John Doe",
...
#products data used to seed the datatbase
product_json = [
	{
...
#recreate_db() resets the database
def recreate_db():
	db.drop_all()
	db.create_all()
	db.session.commit()

#seeder() add the data after resetting the database
def seeder():
	...
	with open("app/editor_data/order_items.csv", "r") as f:
		...
		for row in csvreader:
			try:
				...
				pass
# to declare a command called rsd which calls both recreate_db() and seeder()
@cli.command()
def rsd():
